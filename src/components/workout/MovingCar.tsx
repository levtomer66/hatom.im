  ...
  useEffect(() => {
    ...
    try {
      ...
      if (seen > 0 && state.level > seen) {
        setLevelUp(true);
        timer = setTimeout(() => setLevelUp(false), LEVEL_UP_FLASH_MS);
      }
  ...
  return (
    ...
    >
      ...
      <div
        ...
        aria-valuetext={`${formatMeters(progressM)} / ${formatMeters(targetM)} m`}
      >
        <div className="car-track-fill" style={{ width: `${pct}%` }} />
        <span className="car-track-car" style={{ insetInlineStart: `${pct}%` }} aria-hidden="true">
          {CAR_EMOJI}
        </span>
      ...
      <div className="car-strip-foot">
        <span className="car-strip-distance">
          <strong>{formatMeters(progressM)}</strong> / {formatMeters(targetM)} m
        </span>
